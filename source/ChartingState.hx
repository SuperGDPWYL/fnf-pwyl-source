...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		rightIcon = new HealthIcon('corrupt');
		...
		FlxG.save.bind('corrupt', 'SuperGD');
	...
	function addSongUI():Void
	{
		...
		{
			var songToLoad:String = _song.song;
			if(_song.song.toLowerCase() == 'anarchist')
				{
					songToLoad = 'fuck-you-bitch';
				}
			loadSong(songToLoad);
		});
		...
		{
			var toLoad:String = _song.song.toLowerCase();
			if(_song.song.toLowerCase() == 'anarchist')
				{
					toLoad = 'fuck-you-bitch';
				}
			loadJson(toLoad);
		});
	...
	function addSectionUI():Void
	{
		...
		check_dodge = new FlxUICheckBox(10, 45, null, null, "Dodge section", 100);
		check_dodge.name = 'check_dodge';
		check_dodge.checked = false;
		...
		tab_group_section.add(check_dodge);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Dodge section':
					_song.notes[curSection].dodge = check.checked;
			}
		}
	...
	function updateHeads():Void
	{
		if (check_mustHitSection.checked)
		{
			...
			rightIcon.animation.play('corrupt');
		}
		else
		{
			leftIcon.animation.play('corrupt');
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4,null,false,i[3],true);
