...
class Alphabet extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1, ?dashes:Bool = false)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var spaceChar:Bool = (character == " " || character == "_" || character == "-");
			if (spaceChar)
			{
				consecutiveSpaces++;
			}
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			var spaceChar:Bool = (splitWords[loopNum] == " " || splitWords[loopNum] == "_" || splitWords[loopNum] == "-" && !dashes);
			if (spaceChar)
			{
				consecutiveSpaces++;
			}
